...
These steps walk you through building the EMOD source code from Ubuntu Docker images on a Linux host machine running Linux for Ubuntu 22.04. Before following these steps you must meet the prerequisites in dev-install-ubuntu-docker. Although these steps are specific to the `emod-ubuntu-build` image, you can use them for the other images by replacing "emod-ubuntu-build" with the desired image name, such as "emod-ubuntu-runtime".
...
For more information, see dev-install-build-emod.
...
information, see emod-generic:software-simulation-cli for
generic, emodpy-hiv:emod/software-simulation-cli for HIV,
or emodpy-malaria:emod/software-simulation-cli for malaria.
